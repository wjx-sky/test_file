...
# 学习C++第八天，7月30日
## 类型别名
[传统的方法](typedef)：typedef 类型名 类型别名； 
[C的方法](#define)：#define 类型别名 类型名；
[C++11的方法](using)：using 类型别名 = 类型名；
...
const int *p = &ci;
auto f = *p;//f是int 类型
auto g = &ci;//由于ci是常量，故为底层const,故g是指向常量的指针；，
const auto g = *p;//g是const int类型对象
auto & ce = ci;//const int &引用类型；
 ...
 **目录：**
 ...
 for (int i =0;i<2;++i)
 {
    std::cout<<"学生学号： "<<arrray2[i].num<<
    "  学生ID： "<<array2[i].name<<
    "  学生地址: "<<array2[i].address<<std::endl;
 }

student *stu_1 = &student_1;//定义结构体指针；
stu_1->num = 132;//结构体指针访问结构体成员用->
...
## 结构体
- 结构体指针
定义：`student *student_1 = nullptr;`  
使用：`student_1->name = "lihua“;`
- 结构体类型别名
C语言风格：
```C++
...
- 函数参数传递
1. 通过值传递
`void ptintInfo(point p){};//存在拷贝赋值的开销；
2. 通过引用方式传递
```C++
struct point {
    int x;
...
void movepoint(point &p,int dx,dy){
    p.x = dx;
    p.y = dy;
}；//通过引用传递参数，传递的结构体对象p是是结构体变量的别名，此时的修改会影响结构体对象成员变量的值
void movepoint_c(point p){
    std::cout<<p.x<<std::endl;
}；//值传递，拷贝传递会增大开销，且局部变量的修改不会影响结构体对象成员变量的值；
```
3. 利用指针通过地址传递
影响对象成员变量的值
```C++
...
- 返回结构体数据
```C++
point movepoint3(point *p,int x,int y){
    point p2 = *p;
    p2.x += x;
    p2.y += y;
    return p2
};
point movepoint4(point *p,int x,int y){
    (*p).x += x;//（*p)解引用并取出指针所指向的对象；
    (*p).y += y;
    return *p;
}
point p1 = {12,12};
auto p3 = movepoint4(&p1,1,2);//此时p1和p3相同都是（13,14）
```
## using命名空间
1. using namespace std;
2. std::cout<<...;
3. using std::cout;//之后默认cout均为std::cout;
**头文件中建议不要使用头文件声明**
## 1.字符串类

由一系列字符组成的序列

- c风格的字符串：基于字符数组，以\o结尾
- C++风格字符串：std::string 功能丰富的字符串类，封装了复杂的字符操作。
...
std::string 是定义在<string>头文件中的类，并且提供了丰富的的操作函数；
...
std::string s1(s2);
std::string s1("vlaue");
std::string s1(n,'c');//s1为由连续n个c组成的字符串；

std::string s1(s2,0,4);//利用s2字符串从第0个开始，取四个字符串初始化s1。部分初始化；

```
- 字符串的输入与输出
```C++
std::string str1;
std::cout<<"请输入第一个字符<<std::endl;
std::cin>>str1;
std::cout<<"第一个字符串/n";
//os输出流对象，
...
## 字符串
- 高级用法
字符串流，<sstream>头文件中提供的一个类，用于在内存中进行字符串的读写操作或者类型转化操作
```C++
std::stringstream ss
//主要用于字符串与其他数字类型的转换
//1.字符串流
std::string str_1 = "123";
std::stringstream ss(str_1);//只有初始化可以绑定
int a;
ss>>a;
std::cout<<a+10;
//2.利用函数，C++11及以上；
int num_1 = std::stoi(numstr);
double num_2 = std::stod(numstr);
//其他类型转字符串 std::string str_2 = std::to_string(a);
//其它类型转换为字符串
std::stringstream ss;
ss<<"value: "<<10;
...
//分割字符：
std::string value = "hello my world!";
